...
export const createApi = async (req, res) => {
  try {
    const { owner, ownerClerkId, name, description, isPublic, category, tags } = req.body;

    // Validate required fields
    if (!owner || !ownerClerkId || !name) {
      return res.status(400).json({
        success: false,
        message: "Owner, ownerClerkId, and name are required",
      });
    }
